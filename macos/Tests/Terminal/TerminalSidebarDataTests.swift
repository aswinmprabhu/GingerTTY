...
struct TerminalSidebarDataTests {
    ...
    @Test
    func fetchLocalRepositoryStateIncludesChangesCommitsAndFileList() async throws {
        try await withTemporaryDirectory { temporaryRoot in
            let repository = temporaryRoot.appendingPathComponent("local-state", isDirectory: true)
            try await createRepository(at: repository)
            ...
            let context = try await service.resolveContext(for: repository.path)
            let state = try await service.fetchLocalRepositoryState(for: context, preferredBaseBranch: nil)

            #expect(state.changeSummary.baseBranchName == "main")
            #expect(state.commitEntries.count == 1)
            #expect(state.commitEntries.first?.subject == "Committed sidebar changes")
            #expect(state.filePaths.contains("README.md"))
            #expect(state.filePaths.contains("notes.txt"))
        }
    }
    ...
    func resolveWatchTargetsIncludesRepositoryAndGitDirectories() async throws {
        try await withTemporaryDirectory { temporaryRoot in
            ...
            let context = try await service.resolveContext(for: repository.path)
            let targets = try await service.resolveWatchTargets(for: context)

            #expect(targets.repositoryRoot == repository.standardizedFileURL.path)
            #expect(targets.gitDirectory.hasSuffix("/.git"))
            #expect(targets.gitCommonDirectory.hasSuffix("/.git"))
            #expect(targets.watchedPaths.contains(repository.standardizedFileURL.path))
